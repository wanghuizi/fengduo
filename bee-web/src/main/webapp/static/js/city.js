...
function   select()   {
    with (document.getElementById("projectProvince")){
    ...
    for (i = 0; i < where.length; i++) {
        if   (where[i].loca   ==   loca2)   {
            ...
            for (j = 0; j < loca3.length; j++) {
                with (document.getElementById("projectCity")) {
                    options[0].text   =   "请选择城市";
                    options[0].value   = "";
                    length = loca3.length;
...
function   initt()   {
    ...
    var arr = new Array();
    var projectLocalTemp=$("#projectLocal").val();
    if(projectLocalTemp!=null&&projectLocalTemp!=""){
        arr= projectLocalTemp.split("|");
        var province=arr[0];
        var city=arr[1];
        $("#projectProvince").val(province);
        with (document.getElementById("projectCity")) {
